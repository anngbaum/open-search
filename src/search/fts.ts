...
export async function searchFTS(
  ...
  const tsqueryParam = `$1`;
  clauses.push(`m.text_search @@ websearch_to_tsquery('english', ${tsqueryParam})`);
  ...
  const sql = `
    SELECT * FROM (
      SELECT DISTINCT ON (m.id)
        ...
        ts_rank(m.text_search, websearch_to_tsquery('english', ${tsqueryParam})) as score,
  ...
  const allParams = [query, ...params, options.limit, options.offset];

  const result = await db.query(sql, allParams);

  // Re-sort by score descending after DISTINCT ON, break ties by recency
